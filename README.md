...
1. `--ping`으로 링크를 확인한다. `BOOT_INFO`가 5764 counts/rev를
   보고하면 정상이다.
2. 차체를 들어 바퀴를 띄운 상태로 `--raw 300 0`을 실행한다.
   좌측 바퀴만 전진 방향으로 회전해야 한다.
3. 반대로 돌면 `motor.c`의 `pwm_write()` 내부 두 줄을 교체한다.
   배선은 변경하지 않는다.
4. `--raw 0 300`으로 우측도 동일하게 확인한다.
5. 텔레메트리의 `spd_l`, `spd_r` 부호가 듀티 부호와 일치하는지 본다.
   불일치하면 해당 축의 `ENC_x_INVERT`를 반전시킨다.
6. `--step 150`으로 스텝 응답을 캡처하여 PID를 튜닝한다.
